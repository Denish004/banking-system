...
import { Container, Row, Col, Card, Table, Button, Modal, Form, Alert, Nav } from 'react-bootstrap';
    ...
    const loadData = async () => {
      try {
        ...
        const [userRes, accountsRes, transactionsRes] = await Promise.all([
          ...
          axios.get('http://localhost:5000/api/accounts/transactions', {
            headers: { Authorization: token }
          })
        ]);
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const transactionsRes = await axios.get(
          'http://localhost:5000/api/accounts/transactions',
          {
            headers: { Authorization: token }
          }
        );
